...
GMM <- function (dir, outlines_filt, outlines.data_filt, data_metrics) {
    ...
    png(paste0(dir,"R_statistics/GMM/GMM_panel.png"), width=8333, height=5156, res=1200)
    panel(outlinefile_filt,fac='site')
    
    dev.off()

    efourierfile <- efourier(outlinefile_filt, norm = FALSE)

    efourierfilepca <- PCA(efourierfile)
    str(efourierfilepca)

    png(paste0(dir,"R_statistics/GMM/GMM_scatter_16.png"), width=8333, height=5156, res=1200)
    plot(efourierfilepca$x , pch=16, col=rgb(0,0,1,.4), cex=2)
    text(efourierfilepca$x, labels=outlines.data_filt$ID, cex= 0.7)
    
    dev.off()

    png(paste0(dir,"R_statistics/GMM/GMM_scatter_21.png"), width=8333, height=5156, res=1200)
    ...
    # by barplot
    png(paste0(dir,"R_statistics/GMM/GMM_silhoutte_bar.png"), width=8333, height=5156, res=1200)
    ...
    koptcol <- rainbow("lightblue", "purple4") # over the rainbow
    koptcol <- adjustcolor(koptcol, 0.2) # very transp cols

    clusloeskopt  <- cutree(tree = cluswardd2, k=kopt) # using the wardd2 dendrogr

    png(paste0(dir,"R_statistics/GMM/GMM_scatter.png"), units="px", width=8333, height=5156, res=1200)
    ...
    legend("topright", title="Cluster", c("1","2"), cex=.8, horiz = T, inset = c(0.05, 0.05), fill = koptcol[clusloeskopt])
    ...
    png(paste0(dir,"R_statistics/GMM/GMM_clust.png"), width=8333, height=5156, res=1200)
    tps_iso(cluster_1, cluster_2,iso.nb=200,cont.col = c("darkred", "royalblue"))
    ...
    png(paste0(dir,"R_statistics/GMM/GMM_scatter_1.png"), units="px", width=8333, height=5156, res=1200)
    plot(efourierfilepca, ordered (efourierfilepca$clust,levels=c(1,2)), xax = 1, yax = 2, points = T, cex = 0.5,center.origin = F, grid = TRUE, col = c("darkred", "royalblue"), morphospace = T, ellipses = T, conf.ellipses = 0.95, chull = F, chull.filled = F, chull.filled.alpha = 0.92, eigen = FALSE, rug = FALSE, title = "", labelsgroups=TRUE, cex.labelsgroups=1.15, rect.labelsgroups = FALSE, color.legend = T, old.par = F, nb.grids = 1)
    par(mar=c(5.1, 4.1, 4.1, 8.1), xpd=TRUE)
    ...
    png(paste0(dir,"R_statistics/GMM/GMM_kmeans.png"), units="px", width=8333, height=5156, res=1200)
...
GMM_function <- function(dir, data_metrics) {
