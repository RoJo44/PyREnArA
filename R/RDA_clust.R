### TITLE: PyREnArA (RDA for GMM clusters) ###
...
RDA_clust_function <- function(directory, directory_to_data, parameter, clust) {
    ...
    print(paste('annova cca clust ', clust, ': '))
    print(anova_cca)
    write.csv(anova_cca,paste0(directory, "R_statistics/RDA/",'anova_cca_clust', clust, '.csv'))
    ...
    write.csv(rda_summary,paste0(directory, "R_statistics/RDA/",'rda_summary_clust', clust, '.csv'))
    summary(res)$cont$importance[2,1]
    print(paste('explained proportion clust ', clust, ': ', explained_portion))
    ...
    png(file=paste0(directory, "R_statistics/RDA/",'RDA_Eigenvalue_clust', clust, '.png'), width=8333, height=5156, res=1200)
    ...
    # plot
    png(file = paste0(directory, "R_statistics/RDA/",'RDA_Triplot_clust', clust, '.png'), width=8333, height=5156, res=1200)
    ...
    # plot
    png(file=paste0(directory, "R_statistics/RDA/", 'RDA_per_trait_clust', clust, '.png'), width=8333, height=5156, res=1200)
    ...
    write.csv(datfram, paste0(directory, "R_statistics/RDA/", 'data_metrics_rda_clust_', clust, '.csv'), fileEncoding ="UTF-8")
