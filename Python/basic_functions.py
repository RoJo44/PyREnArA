...
def combined_metrics(directory):
    ...
    for filename in os.listdir(directory):
        ...
        if filename.endswith('color.tif'):
            ...
            cnts, img, blank_image = import_img(directory + name + '.tif')
            ...
            # extract metrics
            ref_Obj = scale(cnts)
            ID, site, width, length, l_w_index, area, percentage_area, contour_length, area_contour_ratio, longest_part_location, widest_part_location, lower_part_length, lower_part_relative, upper_part_length, upper_part_relative, upper_lower_ratio, left_part_width, left_part_relative, right_part_width, right_part_relative, left_right_ratio, width_1, width_2, width_3, width_4, width_5, width_6, width_7, length_1, length_2, length_3, length_4, length_5, tip_angle, midpoint_x_offset, midpoint_y_offset, all_angles = metrics(img_rotated_1, name, rotated_cnts, ref_Obj, folder_directory + name + '_show_metrics.tif')

            # choose metrical data of interest and export
            metrics_data = zip(ID, site, width, length, l_w_index, area, percentage_area, contour_length, area_contour_ratio, longest_part_location, widest_part_location, lower_part_length, lower_part_relative, upper_part_length, upper_part_relative, upper_lower_ratio, left_part_width, left_part_relative, right_part_width, right_part_relative, left_right_ratio, tip_angle, midpoint_x_offset, midpoint_y_offset)
...
def metrics(img, name, cnts, ref_Obj, show_img_name):
    ...
    for cnt in cnts:
        ...
        # extract sitename
        sitename = sitename_extraction(name)
    ...
    return ID, site, width, length, length_width_index, area, percentage_area, contour_length, area_contour_ratio, longest_part_location, widest_part_location, lower_part_length, lower_part_relative, upper_part_length, upper_part_relative, upper_lower_ratio, left_part_width, left_part_relative, right_part_width, right_part_relative, left_right_ratio, width_1, width_2, width_3, width_4, width_5, width_6, width_7, length_1, length_2, length_3, length_4, length_5, tip_angles, midpoint_x_offset, midpoint_y_offset, all_angles
...
# extract sitename from ID
def sitename_extraction(name):
    
    site_name = name.split('_')[0]
    site_layer = name.split('_')[1]
    
    if site_layer[0] == '0':
        site_layer = ''
        site = site_name
    else:
        site = site_name + '_' + site_layer
