...
# generate plots of metrics against parameter of choise
def metrics_vs_plot(directory_to_metrics, directory_to_save, parameter):
    ...
    for col in df.columns:
        ...
        if col == 'ID' or col == 'site' or col == 'ka_cal_BP' or col == 'long' or col == 'lat' or col == 'o_y' or col == 'region' or col == 'typology':
            continue
...
# calculate standard deviation for each instnace of choosen parameter and plot it
...
# calculate mean metrics for each instance of choosen parameter
...
# helperfunction for pearson correlation
def pearsonr_pval(x,y):
    
    return pearsonr(x,y)[1]

# calculate pearson correlation for metrics and choosen parameter
def pearson_correlation(directory_to_metrics, directory_save, parameter):
    ...
    for (colname, colval) in df.items():
        ...
        if colname == '' or colname == 'ID' or colname == 'ka_cal_BP' or colname == 'site' or colname == 'site_layer' or colname == 'typology' or colname == 'region' or colname == 'long' or colname == 'lat' or colname == 'o_y' or colname == 'clust':
            continue
        else:
    ...
    # barplot of pearson correlation
    sns.set(rc = {'figure.figsize': (20, 10), 'figure.dpi': 600, 'savefig.dpi': 600}, context = {'lines.linewidth': 4}, style = 'whitegrid', font = 'Times New Roman')
    
    df_correl = pd.read_csv(directory_save + 'correlation_metrics_' + str(parameter) + '.csv')
    df_correl = df_correl.sort_values('r')
    
    colors = ["darkred" if y <= 0.05 else 'grey' for y in df_correl['p']]
    correlplot = sns.barplot(data = df_correl, x = df_correl['metric'], y = df_correl['r'], palette = colors)

    correlplot.set_ylabel('r (' + str(parameter) + ')', fontsize = 24, weight = 'bold')
    correlplot.set(xlabel = None)
    correlplot.spines['left'].set_linewidth(5)
    correlplot.spines['bottom'].set_linewidth(5)
    correlplot.tick_params(labelsize = 24)
    
    plt.xticks(rotation = 90)
    
    correlfig = correlplot.get_figure()
    correlfig.savefig(directory_save + 'pearson_correlation_metrics_' + str(parameter) + '.png', bbox_inches = 'tight')
    
    plt.close()
    
# generate pearson pairplot
...
def principal_component_analysis(directory_to_data, directory_save, style = 'site', sorting = 'ka_cal_BP', features = ['width', 'length', 'l_w_index', 'area', 'percent_area', 'contour_length', 'area_contour_ratio', 'upper_to_lower_ratio', 'left_to_right_ratio', 'MP_CM_x_offset', 'MP_CM_y_offset']):
    ...
    sns.set(rc = {'figure.figsize': (20, 10), 'figure.dpi': 600, 'savefig.dpi': 600}, context = {'lines.linewidth': 4}, style = 'whitegrid', font = 'Times New Roman')
    screeplot = sns.barplot(x = np.arange(1, len(pcamodel.explained_variance_ratio_) + 1), y = pcamodel.explained_variance_ratio_, color = 'grey')
    ...
    fig_scree.savefig(directory_save + 'PCA_screeplot.png', bbox_inches = 'tight')
    ...
    sns.set(rc = {'figure.figsize': (20, 10), 'figure.dpi': 600, 'savefig.dpi': 600}, context = {'lines.linewidth': 4}, style = 'whitegrid', font = 'Times New Roman')
    scatterplot = sns.scatterplot(x = finalDf['principal component 1'], y = finalDf['principal component 2'], data = finalDf, 
hue = style, palette = 'Dark2', s = 200, alpha = 0.8, edgecolor = 'black')
    
    scatterplot.set_xlabel('PC 1 ' + str(np.round(PC1_val * 100, 1)) + '%', fontsize = 30, weight = 'bold')
    scatterplot.set_ylabel('PC 2 ' + str(np.round(PC2_val * 100, 1)) + '%', fontsize = 30, weight = 'bold')
    scatterplot.spines['left'].set_linewidth(5)
    scatterplot.spines['bottom'].set_linewidth(5)
    scatterplot.tick_params(labelsize = 25)
    
    # legend
    scatterplot.legend(title = style, markerscale = 2.5)
    
    # legend text and title
    plt.setp(scatterplot.get_legend().get_texts(), fontsize = '25')
    plt.setp(scatterplot.get_legend().get_title(), fontsize = '30')
    
    fig_scatter = scatterplot.get_figure()
    fig_scatter.savefig(directory_save + 'PCA_plot.png', bbox_inches = 'tight')
    ...
    fig_effect.savefig(directory_save + 'PCA_variable_effects.png', bbox_inches = 'tight')
                
    plt.close()
    
    # .csv of the first 3 PCA axes
    pcamodel_3 = PCA(n_components = 3)
    pca = pcamodel_3.fit_transform(x)
    pca_df = pd.DataFrame(data = pca, columns = ['PC_1', 'PC_2', 'PC_3'])
    
    pca_csv_df = pd.DataFrame(list(zip(df['ID'], pca_df['PC_1'], pca_df['PC_2'], pca_df['PC_3'])), columns = ['ID', 'PC_1', 'PC_2', 'PC_3'])
    
    pca_csv_df.to_csv(directory_save + "PCA_3_values.csv", header = True, index = False)
...
# generate dendro cluster plot for choosen parameter and sorting
